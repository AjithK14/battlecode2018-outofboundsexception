...
for x in range(earthMap.width):
  for y in range(earthMap.height):
    ...
    kMap.set(ml,earthMap.initial_initial_karbonite_at(ml))
...
class Node:
    ...
    def heuristic(self):
      if self.mapLocation.planet == bc.Planet.Earth:
        return math.sqrt(self.mapLocation.distance_squared_to(self.goal))+self.depth-((.1)*(earthMap.initial_initial_karbonite_at(self.mapLocation))) #change .1 to a function of current karbonite, and round numbers
      else:
        return math.sqrt(self.mapLocation.distance_squared_to(self.goal))+self.depth-((.1)*(marsMap.initial_initial_karbonite_at(self.mapLocation))) #change .1 to a function of current karbonite, and round numbers
def invert(loc):
  ...
  if mostK>0:#found some karbonite to harvest
    ...
    if len(kLocs)>0:
      ...
      if gc.can_sense_location(dest):
        kAmt = gc.initial_karbonite_at(dest)
        if kAmt==0:
          kLocs.pop(0)
        else:
          fuzzygoto(unit,dest)
...
def checkK(loc):
  ...
  return gc.initial_karbonite_at(loc)
  ...
  for x in range(0, width, radius): 
    for y in range(0, height, radius):
      ...
      if(bc.Location.is_on_map(tempml)):
        weight = gc.initial_karbonite_at(tempml)
        for z in range(radius):
          temp = bc.MapLocation(bc.Planet.Mars, x, y+z)
          tempml2 = bc.Location.new_on_map(temp)
          if tempml2.is_on_planet(bc.Planet.Mars):weight += gc.initial_karbonite_at(tempml2)          
...
def findLandingLoc(tempPlanetMap):
    ...
    for x in range(width):
      for y in range(width):
        temp = bc.MapLocation(bc.Planet.Mars, x, y)
        if not gc.has_unit_at_location(temp):
          posLoc.append((x,y))
