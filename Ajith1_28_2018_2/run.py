  ...
  if mostK>0:#found some karbonite to harvest
    ...
    if len(kLocs)>0:
      ...
      if gc.can_sense_location(dest):
        ...
        if kAmt==0:
          kLocs.pop(0)
        elif (unit.id not in robot_stuck or robot_stuck[unit.id] == False):
          fuzzygoto(unit,dest)
...
def factoryProtocol(unit, first_rocket, earthBlueprintLocations, firstRocketLaunched):
  if unit.unit_type == bc.UnitType.Factory:
    ...
    for item in garrison:
      for d in allDirections:  # good for now, change later
...
def findLandingLoc(tempPlanetMap):
    ...
    for x in range(width):
      for y in range(width):
        ...
        if not gc.has_unit_at_location(temp) and gc.is_passable_terrain_at(temp):
          posLoc.append(temp)
          anyLoc.append(temp)
          
        elif gc.is_passable_terrain_at(temp):
          anyLoc.append(temp)
      ...
      if not unit.location.is_in_space() and not unit.location.is_in_garrison():
        ...
        for other in nearby:
          if gc.can_load(unit.id,other.id):
            ...
            robot_stuck[other.id] = False
            print('loaded into the rocket!')
          else:
            if other.id not in robot_stuck:
              robot_stuck[other.id] = True
  ...
  if unit.unit_type == bc.UnitType.Worker:
    if not unit.location.is_in_garrison(): 
      ...
      if unit.id in whereTo:
        if gc.is_move_ready(unit.id) and (unit.id not in robot_stuck or robot_stuck[unit.id] == False):
        ...
        if not first_rocket and unit.location.is_on_planet(bc.Planet.Earth) and len(myUnits) > 15:
          ...
          if unit.movement_heat() < maxRobotMovementHeat: 
            for blueprintLocation in earthBlueprintLocations: #this system handles multiple blueprints, going to the first one
              ...
              if bdist>1 and bdist<16 and (unit.id not in robot_stuck or robot_stuck[unit.id] == False):
...
def mageProtocol(unit, currentRobotArray, rocketLoc):
  if unit.unit_type == bc.UnitType.Mage:
    if not unit.location.is_in_garrison(): 
      ...
      if unit.id in whereTo and (unit.id not in robot_stuck or robot_stuck[unit.id] == False):
        fuzzygoto(unit.id, whereTo[unit.id])
      ...
      if gc.is_move_ready(unit.id):
        if rocketLoc is not None:
          curLoc = unit.location.map_location()
          if unit.id not in unitToPath or len(unitToPath[unit.id]) + unitToIndex[unit.id] < 0 or gc.has_unit_at_location(bc.MapLocation(curLoc.planet,unitToPath[unit.id][unitToIndex[unit.id]][0],unitToPath[unit.id][unitToIndex[unit.id]][1])) and (unit.id not in robot_stuck or robot_stuck[unit.id] == False):
