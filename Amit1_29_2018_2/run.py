...
class mmap():
  def __init__(self,width,height):
    self.width=width
    self.height=height
    self.arr=[[(0, None)]*self.height for i in range(self.width)];
  def onMap(self,loc):
  ...
  def set(self,mapLocation,tup):
    self.arr[mapLocation.x][mapLocation.y]=tup
  def printout(self):
    print('printing map:')
  ...
  def addDisk(self,mapLocation,r2,val):
    ...
    for loc in locs:
      if self.onMap(loc):
        tup = (self.get(loc)[0]+val, mapLocation)
        self.set(loc, tup)
...
def invert(loc):
  if gc.planet() == bc.Planet.Earth:
    ...
    newx = marsMap.width - loc.x
    newy = marsMap.height - loc.y
    return bc.MapLocation(bc.Planet.Earth, newx, newy)
  ...
  if unit.unit_type == bc.UnitType.Rocket and unit.location.is_on_map():
    ...
    if unit.location.is_on_planet(bc.Planet.Earth):
      ...
      if len(garrison) >= countNeeded:
        ...
        tempLoc = findLandingLoc(tempPlanetMap)
...
def coefficient(unit, dmap, numFactories):
  if gc.round() < 200: #early on in the game
    return 1 + (float(gc.round())/200.0)
  else:
    if gc.karbonite() > 300:
      return 1
    else:
      return 1 + (float(gc.karbonite()/300.0))
...
def replication(unit, numWorkers, round, earthBlueprintLocations):
  currentKarbonite = gc.karbonite()
  curLoc = unit.location.map_location()
  '''workersNeeded = 7 + int(min(round, 300) * .1)
  if gc.planet() == bc.Planet.Mars and gc.round() > 900:
    workersNeeded = 9999999 #infinite basically

  elif gc.planet() == bc.Planet.Mars:
    workersNeeded = 14 + int(round*.05)'''
...
def workerProtocol(unit, earthBlueprintLocations, numWorkers, currentRobotArray, dmap, numFactories):
  ...
  if unit.unit_type == bc.UnitType.Worker:
    if not unit.location.is_in_garrison(): 
      ...
      replication(unit, numWorkers, gc.round(), earthBlueprintLocations)

      if gc.planet() == bc.Planet.Earth:
        #blueprint rocket
        myUnits = gc.my_units()
        stuck = False
        if not first_rocket and unit.location.is_on_planet(bc.Planet.Earth) and len(myUnits) > 15 and gc.round() > 150:
          for q in directions:
        ...
        if unit.location.is_on_planet(bc.Planet.Earth):
          ...
          for adjacent in adjacentUnits:#once you build, you need to take it out of earthBlueprintLocations
            ...
            if gc.can_build(unit.id,adjacent.id):
              ...
              if adjacent.health != adjacent.max_health:
                stuck = True
          ...
          if gc.karbonite() >= coefficient(unit, dmap, earthBlueprintLocations) * bc.UnitType.Factory.blueprint_cost():#blueprint
            if gc.can_blueprint(unit.id, bc.UnitType.Factory, d):
              gc.blueprint(unit.id, bc.UnitType.Factory, d)
              earthBlueprintLocations.append(curLoc.add(d))
          ...
          if gc.is_move_ready(unit.id): 
            minD = 99999999  #infinite basically
            blueprintToGo = None
            for blueprint in earthBlueprintLocations:
              dist = curLoc.distance_squared_to(blueprint)
              if dist < minD:
                blueprintToGo = blueprint
                minD = dist
                return #can't do anything else at this point
            if stuck == False and blueprintToGo is not None:
              fuzzygoto(unit, blueprintToGo)


        if len(kLocs)>0 and gc.is_move_ready(unit.id) and gc.karbonite_at(unit.location.map_location()) == 0: #need to go looking for karbonite
          ...
          if gc.can_sense_location(dest):
            ...
            if kAmt==0:
              ...
              fuzzygoto(unit,dest)
      else: #unit is on Mars 
        resTup = dmap.arr[curLoc.x][curLoc.y]
        inDanger = False
        direction = None
        if dmap.arr[curLoc.x][curLoc.y][0] > 0: #worker is in danger
          dangerLoc = dmap.arr[curLoc.x][curLoc.y][1]
          inDanger = True
          toward = curLoc.direction_to(dangerLoc)
          direction = rotate(toward, 4)

        else:
          minDanger = 9999999 #infinite
          minDangerDirection = None
          if dmap.arr[curLoc.x][curLoc.y][0] > 0:
            inDanger = True
            minDanger = dmap.arr[curLoc.x][curLoc.y][0]
        ...
        if gc.karbonite_at(curLoc) < workerHarvestAmount or (inDanger == True):
          fuzzygoto(unit, curLoc.add(direction))
...
elif gc.planet() == bc.Planet.Mars and touchedMars == True:
    ...
    try:
          ...
          for unit in gc.units():
            if not unit.location.is_in_garrison():
              ...
              if unit.team!=my_team and unit.unit_type != bc.UnitType.Factory and unit.unit_type != bc.UnitType.Rocket:

                dmap.addDisk(unit.location.map_location(), unit.attack_range(), unit.damage())
          ...
          if numRockets == 0 and vrgn == False: #
            first_rocket = False
            firstRocketLaunched = False

          elif numRockets == 0 and vrgn == True:
            ####Ajith enter optimal rocket launch/blueprint time code here
            fakeVar = 1
          ...
          for unit in gc.my_units():
            ...
            if location.is_on_map() == True and location.is_in_garrison() == False:
              ...
              workerProtocol(unit, earthBlueprintLocations, numWorkers, currentRobotArray, dmap, numFactories)
